...
public class CommentApiController {
    ...
    public ResponseEntity<?> commentSave(@Valid @RequestBody CommentDto commentDto, BindingResult bindingResult, @AuthenticationPrincipal PrincipalDetails principalDetails){
        ...
        Comment commment = commentService.댓글쓰기(commentDto.getContent(), commentDto.getImageId(), principalDetails.getUser().getId()); // content,
        return new ResponseEntity<>(new CMRespDto<>(1, "댓글쓰기 성공", commment), HttpStatus.CREATED);
    }
